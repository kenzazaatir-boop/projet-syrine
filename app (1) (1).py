...
def load_model():
    try:
        with open("obesity_model.pickle", "rb") as f:
...
Poids insuffisant · Normal · Surpoids I/II · Obésité I/II/III
    ...
    st.divider()
    if model is None:
        st.error("❌ obesity_model.pickle introuvable !")
    else:
...
if st.button("🔍 Prédire le niveau d'obésité", use_container_width=True):
    if model is None:
        st.error("❌ Modèle non chargé. Vérifiez que obesity_model.pickle est dans le dossier.")
    else:
