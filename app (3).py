...
# Chemin absolu vers le pickle (même dossier que app.py)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_PATH = os.path.join(BASE_DIR, "obesity_model.pickle")
...
if st.button("🔍 Prédire le niveau d'obésité", use_container_width=True):
    if model is None:
        st.error(f"❌ Modèle non chargé. Fichier attendu : {MODEL_PATH}")
    else:
