...
# ── Entraînement embarqué (aucun fichier pickle requis) ──────────────────────
...
def load_model():
    ...
    age    = np.random.randint(14, 61, N)
    gender = np.random.choice([0, 1], N)
    height = np.random.normal(1.70, 0.12, N).clip(1.40, 2.00)
    weight = np.random.normal(75, 20, N).clip(30, 180)
    bmi    = weight / height ** 2

    family    = np.random.choice([0, 1], N, p=[0.3, 0.7])
    favc      = np.random.choice([0, 1], N, p=[0.35, 0.65])
    fcvc      = np.random.uniform(1, 3, N)
    ncp       = np.random.choice([1, 2, 3, 4], N, p=[0.1, 0.2, 0.5, 0.2]).astype(float)
    ch2o      = np.random.uniform(1, 3, N)
    faf       = np.random.uniform(0, 3, N)
    tue       = np.random.uniform(0, 2, N)
    smoke     = np.random.choice([0, 1], N, p=[0.85, 0.15])
    scc       = np.random.choice([0, 1], N, p=[0.7, 0.3])
    food_risk = favc + family

    caec_arr   = np.random.choice(['Always','Frequently','Sometimes','Never'], N, p=[0.05,0.15,0.6,0.2])
    calc_arr   = np.random.choice(['Always','Frequently','Sometimes','Never'], N, p=[0.03,0.1,0.5,0.37])
    mtrans_arr = np.random.choice(
        ['Automobile','Bike','Motorbike','Public_Transportation','Walking'],
        N, p=[0.4, 0.05, 0.05, 0.35, 0.15]
    )

    def get_label(i):
        b = bmi[i] - 0.3*faf[i] + 0.2*food_risk[i] + 0.1*family[i]
        if b < 18.5: return 0
        elif b < 25: return 1
        elif b < 27: return 2
        elif b < 30: return 3
    ...
    labels = np.array([get_label(i) for i in range(N)])
...
if st.button("🔍 Prédire le niveau d'obésité", use_container_width=True):
    ...
    try:
        ...
        label, css, conseil = class_info.get(pred, ("Inconnu", "result-orange", ""))
